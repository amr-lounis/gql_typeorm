...
export const product_photo_set = async (manager: EntityManager, id: string, photo: string): Promise<boolean> => {
    ...
    await manager.save(PPhotos, { productId: id, photo: photpBytes });//save : if not exist overwriting else insert newing() ?? ""
    return true
}
export const product_photo_get = async (manager: EntityManager, id: string): Promise<string> => {
    const p = await manager.findOne(PPhotos, { where: { productId: id } },);
